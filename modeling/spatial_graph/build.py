...
class SpatialGraph(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.LANGUAGE.MODEL != "glove":
            ...
            self.query_key1 = nn.Linear(768,256, bias=True)
            self.query_key2 = nn.Linear(768,256, bias=True)
            self.query_key3 = nn.Linear(768,256, bias=True)
        else:
            self.query_head1 = nn.Linear(512,256, bias=True)
            self.query_head2 = nn.Linear(512,256, bias=True)
            self.query_head3 = nn.Linear(512,256, bias=True)
    ...
    def forward(self, videoFeat, videoFeat_lengths, objects, objects_lengths, humans, humans_lengths, query_info):
        
        if self.cfg.LANGUAGE.MODEL == "glove":
            tokens = query_info["tokens"]
            tokens_lengths = query_info["tokens_lengths"]

            output, _  = self.factor_query(tokens, tokens_lengths)        
            
        else:
            tokens_ids   = query_info["tokens_ids"]
            position_ids = query_info["position_ids"]
            lang_mask    = query_info["lang_mask"]
            tokens_lengths = query_info["tokens_lengths"]

            bert_output = self.bert_model(
            input_ids=tokens_ids, position_ids=position_ids, attention_mask=lang_mask, output_hidden_states=True
            )     

            weights = torch.zeros(len(tokens_ids))
            last_hidden_state = bert_output.last_hidden_state
            first_hidden_state = bert_output.hidden_states[0]
