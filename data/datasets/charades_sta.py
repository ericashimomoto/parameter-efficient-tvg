...
class CHARADES_STA(Dataset):
    ...
    def __init__(self, features_path,
        ...
        if cfg.LANGUAGE.MODEL == "glove":
            ...
            vocab_file_name = f'charades_vocab_{self.min_count}_{self.train_max_length}.pickle'

            self.vocab_file_path = vocab_file_name
            self.create_vocab()

            embeddings_file_name = f'charades_embeddings_{self.min_count}_{self.train_max_length}.pth'
            self.embeddings_file_path = embeddings_file_name
            self.get_embedding_matrix(embeddings_path)
            self.tokenizer = None
        # elif cfg.LANGUAGE.MODEL == "llama":
